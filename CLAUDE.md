...
- **`@scaleway/sdk-client` validates the secret key's UUID format eagerly, at client construction time, not at first request** — constructing the real `@scaleway/sdk-*` client in a class constructor (rather than lazily) would throw for any caller building an API instance with a non-UUID-shaped placeholder token, which several offline tests do since they never exercise SDK-backed methods. All four API classes (`shared/api/index.ts`'s `AccountApi`/`FunctionApi`/`ContainerApi`, `shared/api/registry.ts`'s `RegistryApi`) assign `this.sdkApi` a `createLazySdkApi(...)` Proxy in their constructor for exactly this reason — **never** a `get sdkApi()` prototype accessor: these classes are mixed onto the top-level plugin classes via `Object.assign(pluginInstance, ..., apiInstance)`, which only copies an object's own enumerable _values_ and never sees a prototype-level accessor at all (verified the hard way — see the `createLazySdkApi` comment in `shared/api/sdkClient.ts` for the full incident). A `get sdkApi()` accessor here would silently leave `this.sdkApi` `undefined` on every real plugin instance.
