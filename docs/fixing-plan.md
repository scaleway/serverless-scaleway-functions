...
| ------------------------- | ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `npm install` / `npm ci`  | `bun install`                            | **Still a clean swap — re-verified against the current (post-TS-migration) tree.** Fresh clone, `bun install`: migrates `package-lock.json` automatically, resolves the same 636-package tree, and now **also correctly runs the `prepare` script** (`npm run build` → `tsc`), producing a working `dist/` — the compiled plugin loads via plain `require()` afterward. `argon2`'s native binding still works post-install (hashed a real value successfully). New wrinkle found this pass: Bun blocks 3 transitive postinstall scripts by default (`protobufjs`, `es5-ext`, `unrs-resolver` — none of them this repo's own direct dependencies, none required for anything this plugin actually uses) — listed via `bun pm untrusted`, harmless here, but `bun pm trust` exists if one of them ever mattered. Would move to committing `bun.lock` instead of `package-lock.json` as the source of truth.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `jest` (`npm run test:*`) | `bun test`                               | **Not a drop-in — all three original blockers are now resolved, but a fourth, more serious one was found underneath them.** (1) **Resolved**: the `rewire` blocker is gone — eliminated from the test suite entirely during the TS migration. (2) **Resolved** (Pass M-5b, merged): `@jest/globals` imports dropped repo-wide in favor of Jest's own injected globals; `eslint.config.mjs` declares the Jest globals directly for `tests/**/*.js`. (3) **Resolved 2026-08-26** (`chore/bun-migration` branch): every remaining `export =` file converted — single-class/value files (the plugin/lib entry points) kept `export =` but switched their own imports to `import X = require("Y")`; object-mixin files (all of `shared/api/*` and friends) dropped `export =` for plain named exports. Verified: `bun test tests/shared` now runs without the transpiler's `Cannot use import statement with CommonJS-only features` error — the only remaining failure in that suite is unrelated (see below). (4) **New, open, and more serious than any of the above**: `bun test`/`bun <script>` **crashes the entire Bun process** (`Illegal instruction`, `panic(main thread): unsupported uv function: uv_version_string`) the moment anything requires `dockerode` → `docker-modem` → `ssh2`'s native `sshcrypto.node` NAPI addon — confirmed via a direct `bun test` smoke run that imports `index.ts` (which pulls this in transitively through the deploy path). This is a real, currently-open upstream Bun bug ([oven-sh/bun#18546](https://github.com/oven-sh/bun/issues/18546)), not something fixable in this repo's code, and it's not a per-test failure — it takes down the whole `bun test` process, so it would block running the container-deploy-touching suites (`tests/containers*`, likely `tests/deploy`, `tests/runtimes`) under Bun entirely, not just degrade them. `tests/shared` itself is unaffected only because none of its files happen to import anything that reaches `dockerode` at module-load time. Separately, `bun:test`'s Jest-compatibility shim is also missing at least one API this repo's tests use: `jest.advanceTimersByTimeAsync` (confirmed via the same `tests/shared` run — one test fails with `TypeError: jest.advanceTimersByTimeAsync is not a function`). |
...
- **New blocker found underneath the others, 2026-08-26: a native NAPI module crashes the whole Bun process.** `dockerode@5.0.1` → `docker-modem@5.0.7` → `ssh2@1.17.0` ships a native addon (`sshcrypto.node`) for SSH-based Docker daemon connections. Loading it under Bun 1.3.14 hits an unimplemented libuv shim (`uv_version_string`) and **panics the process** (`Illegal instruction`, core dump) rather than throwing a catchable error — confirmed via `bun test` against a one-line file that just imports `index.ts`. This is upstream ([oven-sh/bun#18546](https://github.com/oven-sh/bun/issues/18546)), unrelated to this repo's own code, and not something a `require()`/`import` change here can route around, since `dockerode` is a real, needed dependency (container builds) and its own dependency chain pulls in `ssh2` regardless of how this repo imports it. Any Bun-run suite that transitively loads the deploy/container code path (which includes just importing `index.ts`, since `deploy/scalewayDeploy.ts` pulls in the container-build chain eagerly) would crash outright, not fail gracefully — this is a hard blocker for a full `bun test` swap today, independent of and more serious than the `export =` fix above.

Net effect: with M-5b and the `export =` fix both landed, `tests/shared` now runs cleanly under `bun test` (modulo one unrelated `bun:test` Jest-shim gap, `jest.advanceTimersByTimeAsync`). A full test-runner swap is blocked on the `ssh2`/NAPI crash above, which is outside this repo's control — worth periodically re-checking against newer Bun releases, but not something to keep working around here.

**Fix, if pursued:** package-manager swap (`bun install`, commit `bun.lock`) is low-risk and could be done as its own small PR any time — CI would need `oven-sh/setup-bun` added alongside/instead of `actions/setup-node` in `.github/workflows/test.yml`, and `npm install -g osls` in that workflow would become `bun install -g osls` (verify `osls` itself installs and runs correctly under Bun's global-install path before committing to this). The test-runner swap (`bun test` replacing `jest`) should stay parked until the upstream `ssh2`/NAPI crash is fixed or worked around — none of this repo's own code can resolve it.
...
- M7 Package-manager swap (`bun install` + `bun.lock`, update CI's setup step) — low-risk, can happen any time. Re-verified 2026-08-26 against the post-TS-migration tree: still clean, and now also correctly runs the `prepare`/`build` script.
- M7 Test-runner swap (`bun test`) — **blocked, not just gated on repo-side work.** The `export =`/`import` conflict that used to gate this is resolved (2026-08-26, `chore/bun-migration`: single-value-export files switched their own imports to `import X = require("Y")`; object-mixin files dropped `export =` for named exports — see the table above for why a uniform `export default` swap was rejected). But a deeper, upstream blocker surfaced once that was fixed: `dockerode`'s `ssh2` native NAPI dependency panics the whole Bun process on load ([oven-sh/bun#18546](https://github.com/oven-sh/bun/issues/18546)), which would take down every Bun-run suite that touches the deploy/container code path. This is not fixable from this repo's side — park the test-runner swap until Bun resolves it upstream, or until `ssh2`'s native addon usage changes.
